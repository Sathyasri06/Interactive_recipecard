document.addEventListener('DOMContentLoaded', () => {
    ...
    toggleIngredients.addEventListener('click', () => {
        ...
        toggleIngredients.textContent = ingredientsList.classList.contains('hidden') ? 'View Ingredients' : 'Hide Ingredients';
    });

    toggleSteps.addEventListener('click', () => {
        stepsList.classList.toggle('hidden');
        toggleSteps.textContent = stepsList.classList.contains('hidden') ? 'View Steps' : 'Hide Steps';
    });
    ...
    function highlightStep(index) {
        Array.from(stepsList.children).forEach((step, i) => {
            step.style.backgroundColor = i === index ? '#C4A484' : 'transparent';
        });
    }
